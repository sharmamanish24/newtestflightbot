...
const { ActivityHandler, MessageFactory, EventFactory } = require('botbuilder');
const { ActivityTypes } = require('botframework-schema');

class EchoBot extends ActivityHandler {
    constructor(conversationState, userState, transcriptStore) {
        super();
        ...
        this.onMessage(async (context, next) => {
            ...
            if (context.activity.text.toLowerCase().includes('transfer') && context.activity.text.toLowerCase().includes('call')) {
                const replyText = `Please wait, while I am transferring your call to an agent with DialogId:${context.activity.id}`;
                ...
                const handoffEvent = EventFactory.createHandoffInitiation(context, { "skill": "agent" }, transcript);
        ...
        this.onMembersAdded(async (context, next) => {
            ...
            // // console.log("The member added " + membersAdded.length);
            // for (let cnt = 0; cnt < membersAdded.length; ++cnt) {
            //     if (membersAdded[cnt].id !== context.activity.recipient.id) {
            //         await context.sendActivity(MessageFactory.text(welcomeText, welcomeText));
            //         // await this.run(context, conversationState.createProperty('DialogState'));
            //     }
            // }
            // By calling next() you ensure that the next BotHandler is run.
            await next();
